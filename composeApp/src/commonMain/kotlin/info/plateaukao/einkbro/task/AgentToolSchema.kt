...
//
// Android declares these in data.remote.OpenAiRepository; the iOS
// OpenAiRepository has no tool-calling support yet, so the schema types live
// here until it grows one (gap reported in the porting notes). The JSON shape
// matches OpenAI's `tools` array entries.
...
object AgentToolSchema {
    ...
    val tools: List<ToolDefinition> = listOf(
        ...
        toolDef(
            ...
            description = "Fetch one or more pages and save their reader-mode content as " +
                "chapters of a new EPUB file in the app's saved-EPUB list. Use when the " +
    ...
    const val SYSTEM_PROMPT =
        "You are EinkBro, an automation agent running inside a mobile web browser. " +

...
sealed interface CaptionFetchResult {
    /** Timedtext JSON in the shape the caption pipeline expects. */
...
class YouTubeCaptionFetcher(
    ...
    private val dualCaptionProcessor = DualCaptionProcessor(config)
    private val json = Json { ignoreUnknownKeys = true }

    /**
     * Returns [CaptionFetchResult.Captions] with timedtext JSON (the [TimedText]
     * shape the caption pipeline stores) when a caption track or transcript is
    ...
    private suspend fun fetchPlayerResponse(videoId: String): PlayerResponse? = try {
        val response = client.post(PLAYER_API_URL) {
            contentType(ContentType.Application.Json)
            header("User-Agent", ANDROID_CLIENT_USER_AGENT)
            setBody(playerRequestBody(videoId))
            timeout { requestTimeoutMillis = 10_000 }
        }
        if (response.status.value != 200) null
        else json.decodeFromString(PlayerResponse.serializer(), response.bodyAsText())
    } catch (e: Exception) {
        null
    }
    ...
    private suspend fun requestGeminiTranscript(videoId: String): GeminiOutcome = try {
        ...
        val response = client.post("$GEMINI_API_PREFIX$model:generateContent") {
            ...
            setBody(geminiRequestBody(videoId))
            // Gemini has to ingest the whole video before answering, so this has
            // to override the shared client's 2-minute request timeout.
        ...
        if (code != 200) {
            GeminiOutcome.Error(
                "Gemini ($code): ${parseGeminiError(body)}",
    ...
    companion object {
        ...
        private fun playerRequestBody(videoId: String): String {
            val hl = platformDefaultLanguage().ifBlank { "en" }
        ...
        /**
         * Android splits the URL with java.net.URI; this walks the string instead
         * (the port's [info.plateaukao.einkbro.util.Uri] only resolves the host)
         * but keeps the same host/path/query decomposition.
         */

...
fun GesturePickerScreen(navController: NavHostController) {
    ...
    val expanded: SnapshotStateMap<StringResource, Boolean> = remember { mutableStateMapOf() }
...
@Composable
private fun SelectedHeader(slotTitleResId: StringResource, action: BrowserAction) {
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text(
                text = stringResource(slotTitleResId),
                style = MaterialTheme.typography.caption,
                color = MaterialTheme.colors.onBackground.copy(alpha = 0.7f),
            )
            Text(
                text = stringResource(Res.string.action_selected_label) +
...
@Composable
private fun CategoryHeader(titleResId: StringResource, expanded: Boolean, onClick: () -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }

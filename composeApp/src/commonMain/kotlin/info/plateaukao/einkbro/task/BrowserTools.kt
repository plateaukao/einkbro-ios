...
/**
 * Tool façade over EinkBro primitives (off-screen web engine + OpenAI repository).
 *
 ...
 *
 * All navigation methods operate on a dedicated off-screen [WebViewEngine] — the
 * user's active tab is never disturbed. Cookies are shared process-wide via the
 * WKWebsiteDataStore, so auth-walled sites work automatically.
 */
...
data class InitialPageSnapshot(
    ...
    val rawHtml: String = "",
    /** Resolver for the live originating tab's engine, so the agent can run JavaScript
     *  on it with user-visible effect (e.g. test a removal selector before persisting
     *  it). A provider — not a hard reference — because the user may close that tab
     *  while the agent chat is still open; return null once it's gone. (Android used a
     *  WeakReference<EBWebView> for the same reason.) */
    val originEngineProvider: (() -> WebViewEngine?)? = null,
)
...
interface BrowserTools {
    ...
    // snapshot lookups are zero-cost — no web engine access.
     ...
     * before any JavaScript ran (view-source equivalent). The last result is cached
     * per-URL so paged tool reads don't re-download. Returns null on network/HTTP
     * failure or non-http(s) URLs.
     */
    ...
    /** Evaluates [code] in the off-screen engine and returns the completion value
     *  (stringified; use JSON.stringify in the script for structured results).
     ...
     *  DOM changes are immediately visible to the user. Returns the stringified
     *  completion value, or null when that tab has been closed. */
    suspend fun runJavascriptInInitialTab(code: String): String?

    // ── Navigation / content ────────────────────────────────────────────

    /** Loads [url] in the off-screen engine and awaits page load. Returns false on timeout. */
    suspend fun openUrlInBg(url: String): Boolean

    /** Loads the user's configured search engine with [query] in the off-screen engine.
     *  Follow with [currentBgPageLinks]/[currentBgPageText] to read the results. */
    suspend fun searchInBg(query: String): Boolean

    /** Reader-mode text of the off-screen engine's current page (empty if none loaded). */
    suspend fun currentBgPageText(): String

    /** Extracted `{text, href}` links from the off-screen engine's current page. */
    ...
    /**
     * Human-readable name of the app's current UI locale (e.g. "Chinese", "English").
    ...
    /**
     * Speak [text] via the shared TTS view model. Fire-and-forget — if no article is
     * currently being read, TTS starts immediately; otherwise [text] is appended to
     * the TTS queue and played after the current utterance finishes. [title] labels
     * the utterance and has no effect on spoken output.
     */
    ...
    /**
     * Fetches each chapter URL in the off-screen engine, extracts its reader-mode
     * XHTML, and writes all chapters into a new EPUB under the app's Documents
     ...
     * successful one. Returns the saved file path, or null when nothing could be
     * saved.
     */
    ...
    /** Releases the off-screen engine. Called by TaskRunner when done. */
    fun dispose()

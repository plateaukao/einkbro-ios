...
/**
 * Per-site settings, resolved per URL through a chain of rules (port of the
 * Android DomainConfigManager).
 *
...
class DomainConfigManager(
    ...
    var domainConfigurationMap: MutableMap<String, DomainConfigurationData> = mutableMapOf()
    ...
    // region per-site display / behavior overrides (null = use global setting)
    ...
    fun getEnableAdBlock(url: String): Boolean = resolve(url) { it.enableAdBlock } ?: browser.adBlock

    fun getEnableCookies(url: String): Boolean = resolve(url) { it.enableCookies } ?: browser.cookies
    ...
    /** The host rule for [url] (created empty when none exists) — legacy accessor. */
    fun getDomainConfig(url: String): DomainConfigurationData {
        val host = SiteRuleKey.hostOfUrl(url) ?: return DomainConfigurationData("")
        return getRuleOrNew(host)
    }

...
/**
 * Entry composable; was TtsSettingDialogFragment.Content(). The stub view
 * model is pre-seeded to look mid-reading (sentence 3/12, one queued article).
 */
@Composable
fun TtsSettingDialogContent(
    ...
    // The Android original stacks ETtsVoiceDialogFragment on the fragment
    // manager; here the same content opens in a nested Compose dialog.
    ...
    Column(
        ...
        if (readingState.value != IDLE && showReadingContext.value) {
            ...
            MainTtsSettingDialog(
                ...
                showEttsVoiceDialog = { showEttsVoiceDialog.value = true },
        ...
        TtsDialogButtonBar(
            ...
            gotoSettingAction = { IntentUnit.gotoSystemTtsSettings(context) },
...
private fun ReadingSpeedSelection(selectedSpeedValue: Int, onSpeedValueClick: (Int) -> Unit) {
    Text(
        stringResource(Res.string.read_speed),
...
private fun SystemLanguageSelection(selectedLocale: Locale, showLocaleDialog: () -> Unit) {
    Text(
        stringResource(Res.string.setting_tts_locale),
        modifier = Modifier.padding(vertical = 6.dp),
        color = MaterialTheme.colors.onBackground,
        style = MaterialTheme.typography.h6,
        fontWeight = FontWeight.Bold,
...
private fun ETtsVoiceSelection(
    ...
    SelectableText(
        modifier = Modifier.padding(horizontal = 1.dp, vertical = 3.dp),
        selected = false,
        text = LocalContext.current.getString(Res.string.other_voices)
    ) {
        showEttsVoiceDialog()
    }
}
...
private fun VoiceTitle() {
    Text(
        stringResource(Res.string.setting_tts_voice),
        modifier = Modifier.padding(vertical = 6.dp),
        color = MaterialTheme.colors.onBackground,
        style = MaterialTheme.typography.h6,
        fontWeight = FontWeight.Bold,
...
private fun TtsEngineTitle() {
    Text(
        stringResource(Res.string.setting_tts_type),
        modifier = Modifier.padding(vertical = 6.dp),
        color = MaterialTheme.colors.onBackground,
        style = MaterialTheme.typography.h6,
        fontWeight = FontWeight.Bold,
        ...
        Row(
            ...
            if (ttsType == TtsType.SYSTEM) {
                TextButton(
                    ...
                    Text(
                        stringResource(Res.string.system_settings),
                        color = MaterialTheme.colors.onBackground
                    )
                }
            }
